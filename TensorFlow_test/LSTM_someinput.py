...
class Bi_LSTM_model:
    def __init__(self):
        ...
        self.num_hidden2 = 20
        self.num_classes = 2
    def BiRNN1(self,x, weights, biases):
        x1 = tf.unstack(x, self.steps, 1)
        lstm_fw_cell1 = rnn.BasicLSTMCell(self.num_hidden1, forget_bias=1.0,name='lstm_fw_cell1')
        # Backward direction cell
        lstm_bw_cell1 = rnn.BasicLSTMCell(self.num_hidden1, forget_bias=1.0,name='lstm_bw_cell1')

        # Get lstm cell output
        try:
            outputs2, _, _ = rnn.static_bidirectional_rnn(lstm_fw_cell1, lstm_bw_cell1, x1,
                                                         dtype=tf.float32)
        except Exception:  # Old TensorFlow version only returns outputs not states
            outputs2 = rnn.static_bidirectional_rnn(lstm_fw_cell1, lstm_bw_cell1, x1,
                                                   dtype=tf.float32)

        # Linear activation, using rnn inner loop last output
        return tf.matmul(outputs2[-1], weights['out1']) + biases['out1']

    def BiRNN2(self,x, weights, biases):
        x2 = tf.unstack(x, self.steps, 1)
        lstm_fw_cell2 = rnn.BasicLSTMCell(self.num_hidden2, forget_bias=1.0,name='lstm_fw_cell2')
        # Backward direction cell
        lstm_bw_cell2 = rnn.BasicLSTMCell(self.num_hidden2, forget_bias=1.0,name='lstm_bw_cell2')

        # Get lstm cell output
        try:
            outputs2, _2, _2 = rnn.static_bidirectional_rnn(lstm_fw_cell2, lstm_bw_cell2, x2,
                                                         dtype=tf.float32)
        except Exception:  # Old TensorFlow version only returns outputs not states
            outputs2 = rnn.static_bidirectional_rnn(lstm_fw_cell2, lstm_bw_cell2, x2,
                                                   dtype=tf.float32)

        # Linear activation, using rnn inner loop last output
        return tf.matmul(outputs2[-1], weights['out2']) + biases['out2']

    def main(self,texts_train1,texts_test1,texts_train2,texts_test2,target_train,target_test):
        X1 = tf.placeholder("float", [None, self.steps,self.num_input1])
        X2 = tf.placeholder("float", [None, self.steps,self.num_input2])
        Y = tf.placeholder("float", [None, self.num_classes])
        weights1 = {
            # Hidden layer weights => 2*n_hidden because of forward + backward cells
            'out1': tf.Variable(tf.random_normal([2 * self.num_hidden1, self.num_classes]))
        }
        biases1 = {
            'out1': tf.Variable(tf.random_normal([self.num_classes]))
        }

        weights2 = {
            # Hidden layer weights => 2*n_hidden because of forward + backward cells
            'out2': tf.Variable(tf.random_normal([2 * self.num_hidden2, self.num_classes]))
        }
        biases2 = {
            'out2': tf.Variable(tf.random_normal([self.num_classes]))
        }

        logits1 = Bi_LSTM_model().BiRNN1(X1, weights1, biases1)
        logits2 = Bi_LSTM_model().BiRNN2(X2, weights2, biases2)
        ...
        # Start training
        training_steps=texts_train1.shape[0]//self.batch_size
        with tf.Session() as sess:
            # Run the initializer
            sess.run(init)

            for i in range(training_steps):
                start = (i * self.batch_size) % texts_train1.shape[0]  #
                end = start + self.batch_size
                batch_len = texts_train1[start:end].shape[0]
                batch_x1 = texts_train1[start:end].reshape((batch_len, self.steps, self.num_input1))
                batch_x2 = texts_train2[start:end].reshape((batch_len, self.steps, self.num_input2))
                batch_y = target_train[start:end].reshape((batch_len, self.num_classes))
                # Run optimization op (backprop)
                sess.run(train_op, feed_dict={X1: batch_x1,X2:batch_x2, Y: batch_y})
                if i % self.display_step == 0 or i == 1:
                    # Calculate batch loss and accuracy
                    loss, acc = sess.run([loss_op, accuracy], feed_dict={X1: batch_x1,X2:batch_x2,
                                                                         Y: batch_y})
            ...
            test_data1 = texts_test1[:len1]
            test_data2 = texts_test2[:len1]
            test_y = target_test[:len1]
            print("test accuracy %g" % sess.run(accuracy, feed_dict={X1: test_data1.reshape((len1, self.steps, self.num_input1)),X2:test_data2.reshape((len1, self.steps, self.num_input2)), Y: test_y.reshape((len1, self.num_classes)),}))
